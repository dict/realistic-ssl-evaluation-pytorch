...
parser.add_argument("--validation", default=10000, type=int, help="validate at this interval (default 10000)")
...
parser.add_argument("--csv", "-c", default="", type=str, help="csv name")
args = parser.parse_args()

with open(args.csv, 'w') as logfile:
    logwriter = csv.writer(logfile, delimiter=',')
    logwriter.writerow(['logs'])
...
class VDF(nn.Module):
    def __init__(self, coeff):
        super(VDF, self).__init__()
        self.coeff = coeff

    def forward(self, logits, targets):
        '''
        Compute loss: kl - evi (min kl & max evidence)

        '''
        alphas = torch.exp(logits)
        betas = torch.ones_like(logits)

        # compute evi loss psi(alpha_target) - psi(alpha_zero)
        a_ans = torch.gather(alphas, -1, targets.unsqueeze(-1)).squeeze(-1)
        #a_self = torch.sum(torch.digamma(alphas)*targets,-1)
        a_zero = torch.sum(alphas, -1)
        evi_loss = torch.digamma(a_ans) - torch.digamma(a_zero)
        #evi_loss = a_self - torch.digamma(a_zero)
        ...
        loss = (self.coeff*kl_loss - evi_loss).mean()
...
for l_data, u_data in zip(l_loader, u_loader):
    ...
    target[target==-1] = 0
    #print(target)
    cls_loss = criterion(outputs[:50], target[:50])

    # ramp up exp(-5(1 - t)^2)
    coef = alg_cfg["consis_coef"] * math.exp(-5 * (1 - min(iteration/shared_cfg["warmup"], 1))**2)
    ssl_loss = ssl_obj(inputs[50:], outputs[50:].detach(), model) * coef
    ...
    if iteration == 1 or (iteration % 100) == 0:
        ...
        with open(args.csv, 'a') as logfile:
            logwriter = csv.writer(logfile, delimiter=',')
            logwriter.writerow(["iteration [{}/{}] cls loss : {:.6e}, SSL loss : {:.6e}, coef : {:.5e}, time : {:.3f} iter/sec, rest : {:.3f} min, lr : {}".format(
            iteration, shared_cfg["iteration"], cls_loss.item(), ssl_loss.item(), coef, 100 / wasted_time, rest, optimizer.param_groups[0]["lr"])])
    ...
    if (iteration % args.validation) == 0 or iteration == shared_cfg["iteration"]:
        with torch.no_grad():
            ...
            with open(args.csv, 'a') as logfile:
                logwriter = csv.writer(logfile, delimiter=',')
                logwriter.writerow(["varidation accuracy : {}".format(acc)])
            # test
            if maximum_val_acc < acc:
                ...
                with open(args.csv, 'a') as logfile:
                    logwriter = csv.writer(logfile, delimiter=',')
                    logwriter.writerow(["test accuracy : {}".format(test_acc)])
